"""
ANTLR Shell Grammar Converter

This module defines a converter that transforms the abstract syntax tree (AST)
generated by the ANTLR Shell Grammar Parser into a representation of shell
commands.

The converter constructs a tree of command objects, where each node represents
a different aspect of a shell command, such as pipes, sequences, calls, atoms,
and various types of quoting and redirection.

Classes:
- Converter: Converts the AST into a tree of command objects.

Note: AssertionError should not be reached. If it is, it indicates a bug.
"""
