...
class TestShellCommands(unittest.TestCase):
    ...
    def test_convert_echo_hello(self):
        self.test_convert_check_command("echo hello", Commmand(Call((Argument("echo")), Atom(Argument("hello")))))

    def test_convert_ls_l_home(self):
        self.test_convert_check_command("ls -l /home", Commmand(
            Call((Argument("ls")), Atom(Argument("-l")),
            Atom(Argument("/home")))))

    def test_convert_cat_file_txt_pipe_grep_search_term(self):
        self.test_convert_check_command("cat file.txt | grep search_term", Commmand(Pipe(
    ...
    def test_convert_echo_hello_redirection(self):
        self.test_convert_check_command("echo hello > output.txt", Commmand(Call(
            (Argument("echo")),
            Atom(Argument("hello")),
    ...
    def test_convert_grep_search_term_redirection(self):
        self.test_convert_check_command("grep search_term < input.txt", Commmand(Call(
            (Argument("grep")),
    ...
    def test_convert_mkdir_cd_new_folder(self):
        self.test_convert_check_command("mkdir new_folder; cd new_folder", Commmand(Seq(
    ...
    def test_convert_echo_double_quoted(self):
        self.test_convert_check_command('echo "hello world"', Commmand(Call(
            (Argument("echo")),
            Atom(Argument(Quoted(DoubleQuoted("hello world"))))
        )))

    def test_convert_echo_back_quoted(self):
        self.test_convert_check_command("echo `date`", Commmand(Call(
            (Argument("echo")),
            Atom(Argument(Quoted(BackQuoted("date"))))
        )))
