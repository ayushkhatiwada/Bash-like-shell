...
from expressions import Command, Call, Atom, Argument, Pipe, Seq, Redirection, Quoted, DoubleQuoted, BackQuoted
...
class TestShellCommands(unittest.TestCase):
    ...
    def test_echo_hello(self):
        self.check_command("echo hello", Command(Call((Argument("echo")), Atom(Argument("hello")))))

    def test_ls_l_home(self):
        self.check_command("ls -l /home", Command(
            Call((Argument("ls")), Atom(Argument("-l")),
            Atom(Argument("/home")))))

    def test_cat_file_txt_pipe_grep_search_term(self):
        self.check_command("cat file.txt | grep search_term", Command(Pipe(
            Call((Argument("cat")), Atom(Argument("file.txt"))),
            Call((Argument("grep")), Atom(Argument("search_term")))
        )))

    def test_echo_hello_redirection(self):
        self.check_command("echo hello > output.txt", Command(Call(
            (Argument("echo")),
            Atom(Argument("hello")),
    ...
    def test_grep_search_term_redirection(self):
        self.check_command("grep search_term < input.txt", Command(Call(
            (Argument("grep")),
