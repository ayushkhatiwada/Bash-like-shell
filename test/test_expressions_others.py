...
class TestShellFeatures(unittest.TestCase):
    ...
    def test_call(self):
        ...
        self.assertEqual(
            str(call), "Call(Argument(['echo']), Atom(Argument(['hello'])))"
        )

    def test_atom(self):
        atom = Atom(Argument("echo"))
        self.assertEqual(str(atom), "Atom(Argument(['echo']))")

    def test_redirection(self):
        redirection = Redirection(">", Atom(Argument("output.txt")))
        self.assertEqual(
            str(redirection), "Redirection(>, Atom(Argument(['output.txt'])))"
        )
    ...
    def test_quoted(self):
        ...
        self.assertEqual(
            str(quoted), "Quoted(SingleQuoted(['single_quoted']))")

    def test_single_quoted(self):
        single_quoted = SingleQuoted("single_quoted")
        self.assertEqual(str(single_quoted), "SingleQuoted(['single_quoted'])")

    def test_double_quoted(self):
        double_quoted = DoubleQuoted(["hello", "world"])
        self.assertEqual(
            str(double_quoted), "DoubleQuoted([['hello', 'world']])"
        )

    def test_back_quoted(self):
        back_quoted = BackQuoted(Atom(Argument("command")))
        self.assertEqual(
            str(back_quoted), "BackQuoted(Atom(Argument(['command'])))"
        )
